...
export function PageHero({eyebrow,title,description,action}:{eyebrow:string;title:string;description:string;action?:React.ReactNode}){const variant=resolveHeroVariant(eyebrow);return <section className={`page-hero page-hero--${variant}`}><img className="page-hero__seal" src={officialLogo} alt="" aria-hidden="true"/><div className="page-hero__circuit" aria-hidden="true"><span/><span/><span/><i/><i/><i/></div><div className="container"><p className="eyebrow eyebrow--light">{eyebrow}</p><div className="page-hero__content"><h1>{title}</h1><p>{description}</p>{action}</div></div></section>}
